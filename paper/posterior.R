...
##--------------------------Examing codes-----------------------------
z_m = colMeans(ztrue_mc)
e_rad = colMeans(radgp_pred) - z_m
e_nngp = colMeans(nngp_pred) - z_m
e_nngp_i = colMeans(nngp_pred_i) - z_m
mean(e_rad^2)
mean(e_nngp^2)
mean(e_nngp_i^2)

df_e = data.frame(e_rad, e_nngp, e_nngp_i)


radtrain_nnpred = nngp_obj
radtrain_nnpred$theta = radgp_obj$theta
radtrain_nnpred$w = radgp_obj$w[nngp_obj$ord,]
radtrain_nnpred$nugg = radgp_obj$nugg

radtrain_nnpred_pred = t(predict(radtrain_nnpred, coords_test, mcmc_keep=mc_true, n_threads=16, independent=TRUE)$wout)
wasserstein(pp(ztrue_mc), pp(radtrain_nnpred_pred), p=2)
